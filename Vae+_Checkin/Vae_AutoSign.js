...
(function () {
    ...
    function md5(input) {
        ...
        const padded = new Uint8Array((size + 9 + 63 & ~63));
        ...
        for (let i = 0; i < 8; i++) {
            padded[padded.length - 8 + i] = Number(bits & 255n);
            bits >>= 8n;
        }
        ...
        return [a, b, c, d].map(x => {
            const h = hex(x);
            return h.slice(6) + h.slice(4, 6) + h.slice(2, 4) + h.slice(0, 2);
        }).join('');
    }

    function secureRandom(n) {
        if (!globalThis.crypto || !crypto.getRandomValues) {
            fail('需要 Surge WebView 引擎的安全随机数');
        }
    ...
    function publicKey(raw) {
        ...
        const alg = tlv(der, root.start);
        const bit = tlv(der, alg.end);
        ...
        const seq = tlv(der, bit.start + 1);
        const mod = tlv(der, seq.start);
        const exp = tlv(der, mod.end);

        if (mod.tag !== 2 || exp.tag !== 2) {
            fail('公钥整数格式错误');
        }

        const modulus = num(der.slice(mod.start, mod.end));
        const exponent = num(der.slice(exp.start, exp.end));
        const keySize = Math.ceil(
            mod.end - mod.start - (der[mod.start] === 0 ? 1 : 0)
        );

        if (keySize !== 128 || exponent !== 65537n) {
            fail('此版本只验证了 1024 位 RSA 公钥');
        }
    ...
    function rsaEncrypt(message, key) {
        ...
        const width = key.keySize;
        const max = width - 11;

        const out = new Uint8Array(
            Math.ceil(input.length / max) * width
        );
        ...
        for (
            ...
            const part = input.slice(offset, offset + max);
            const padded = new Uint8Array(width);

            padded[0] = 0;
            padded[1] = 2;

            for (let i = 2; i < width - part.length - 1; i++) {
                let v = 0;
            ...
            padded[width - part.length - 1] = 0;
            padded.set(part, width - part.length);
            ...
            for (let i = width - 1; i >= 0; i--) {
                out[block * width + i] = Number(encrypted & 255n);
                encrypted >>= 8n;
            }
        }
    ...
    function generateQ(action, specific, auth, key) {
        ...
        for (const name of DEVICE_KEYS) {
            if (
                auth[name] !== undefined &&
                auth[name] !== ''
            ) {
                p[name] = String(auth[name]);
            }
        }

        Object.assign(p, specific || {});

        p._authOnce = Array.from(
            secureRandom(8),
            v => 'abcdefghijklmnopqrstuvwxyz0123456789'[v % 36]
        ).join('');
        ...
        const value = rsaEncrypt(
            JSON.stringify(p),
            key
        );

        const index =
            Number.parseInt(auth.userId, 10) % 128;
    ...
    function findTask(root) {
        ...
        if (String(root.taskKey) === '201') {
            return root;
        }

        for (const v of Object.values(root)) {
            const item = findTask(v);
    ...
    function signStats(record) {
        const total = Number(record.totalCount);
        const streak = Number(record.continuity);
    ...
    function sendResult(signedNow, record, rewardText) {
        const stats = signStats(record);
    ...
    async function main() {
        ...
        if (
            ...
            fail(
                '授权资料不完整：请在启用 GetAuth 后重新登录 Vae+ 并打开一次首页'
            );
        }
        ...
        if (
            !status.signRecord ||
            typeof status.signRecord.signToday !== 'boolean'
        ) {
            fail(
                '签到状态响应缺少 signRecord'
            );
